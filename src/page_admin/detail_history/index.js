...
const DetailHistory_admin = ({navigation, route}) => {
    ...
    const button = () => {
        if (status === 'Telah tiba di Laundry') {
            const onConfirmation=()=>{
                Firebase.database().ref(`washing/${data.uidWashing}/`).update({
                    status : 'Proses Pencucian Pakaian',
                    nameAdmin : profile.fullName
                })
                .then(()=>{
                    push_notification(`[${profile.fullName}] Proses Pencucian Pakaian`, data.token)
                    navigation.goBack()
                })
            }
            return (
                ...
                <View style={{paddingHorizontal: 5}}>
                    <Gap height={5}/>
                    <Button title='Konfirmasi Pakaian Dicuci' onPress={onConfirmation}/>
                    <Gap height={5}/>
                </View>
                </>
            )
        }
        if (status === 'Proses Pencucian Pakaian') {
            const onConfirmation=()=>{
                Firebase.database().ref(`washing/${data.uidWashing}/`).update({status : 'Pakaian Selesai Dicuci', nameAdmin : profile.fullName})
                .then(()=>{
                    push_notification(`[${profile.fullName}] Pakaian Selesai Dicuci`, data.token)
                    navigation.goBack()
                })
            }
            return (
                <View style={{paddingHorizontal: 5}}>
                    <Gap height={5}/>
                    <Button title='Pakaian Selesai Dicuci' onPress={()=>onConfirmation()}/>
                    <Gap height={5}/>
                </View>
            )
        }
...
export default DetailHistory_admin
